...
Total Submissions: 9910		Accepted: 3256
...
string manip(string num)
{
   ...
   for(int i=0;i<=58;i++){
		if(buff[i]!=0){
			if(buff[i]>9){
				tmp.push_back(buff[i]/10+48);
				tmp.push_back(buff[i]%10+48);
			}
			else{
			    tmp.push_back(buff[i]+48);
			}
			tmp.push_back(i);
		}
	}
...
int main()
{
	...
	while(cin>>num&&num!="-1"){
	    ...
	    bool flag=false;
		for(k=1;k<=15;k++){
		    string tmp=manip(str[counter]);
		    if(tmp==str[counter]){
		    	if(k==1){
		    		cout<<num<<" is self-inventorying"<<endl;
		    	}
		    	else{
		    		cout<<num<<" is self-inventorying after "<<k-1<<" steps"<<endl;
		    	}
		    	break;
		    }
		    else{
		    	for(int i=0;i<=counter;i++){
		    		if(str[i]==tmp){
		    			cout<<num<<" enters an inventory loop of length "<<k-i<<endl;
		    			flag=true;
		    			break;
		    		}
		    	}
